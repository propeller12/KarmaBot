...
from crud import get_user_by_id, create_user, create_habit, assign_habit, get_todays_tasks, mark_habit_completed, mark_habit_failed
...
@app.route('/fail/<int:log_id>', methods=['POST'])
def fail_task(log_id):
    mark_habit_failed(log_id)
    return redirect(url_for('index'))
